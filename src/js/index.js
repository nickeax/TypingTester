const rawScript = `Marty:  Hello.  My name is Marty DeBergi.  I'm a film maker.  I
...
class CharacterDialog {
	...
	constructor({ name, lines }) {
		this.name = name
		this.lines = lines // string array
	}
}

let scriptArr = rawScript.split('\n')
let dialog = []
let gettingLines = false
let character = ''
let lines = []
...
scriptArr.forEach(l => {
	if (l.indexOf(':') !== -1) {
		if (gettingLines) {
			gettingLines = false
			if (character.length > 0 && l.length > 0) {
				lines = lines.filter(x => x.indexOf(":") === -1);
				dialog.push(new CharacterDialog({ name: character, lines: lines }))
				lines = []
				character = ''
			}
		}
		let tmp0 = l.split(':')[0]
		let tmp1 = l.split(':')[1]
		if (tmp0.length > 0 && character.length === 0) {
			character = l.split(':')[0];
		}
		if (tmp1.length > 0 && tmp1 !== '\n') {
			lines.push(l.split(':')[1].trim())
		}
		gettingLines = true

	}
	if (l.length > 0) {
		l = l.split().filter(x => x !== '\t').join('')
		lines.push(l)
	}
})
...
/*
The condition of a single line containg a colon is causing those lines to be lost
When a colon line is detected, the gettingLines flag is set

If a colon is encountered, the name and first line needs to be separated from that line
The issue is when the very next line also contains a colon, as it is skipped.
